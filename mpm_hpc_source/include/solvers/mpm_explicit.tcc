...
template <unsigned Tdim>
void mpm::MPMExplicit<Tdim>::compute_critical_timestep_size(double dt) {
  ...
  console_->info("Critical time step size for elastic wave propagation (solid base) is {} s", critical_dt);
...
bool mpm::MPMExplicit<Tdim>::solve() {
  ...
  this->write_reaction_force(true,  this->step_ + 1, this->nsteps_);
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::pre_process() {
  bool status = true;
  console_->info("MPM analysis type {}", io_->analysis_type());
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::get_deformation_task() {
  bool status = true;
  if (step_ < nsteps_) {
...
template <unsigned Tdim>
void mpm::MPMExplicit<Tdim>::get_info(unsigned& dim, bool& resume,
...
template <unsigned Tdim>
void mpm::MPMExplicit<Tdim>::get_status(double& dt, unsigned& step,
                                        unsigned& nsteps,
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::send_deformation_task(
...
// Set particle stress
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::set_stress_task(const Eigen::MatrixXd& stresses,
                                             bool increment) {
  bool status = true;
  if (step_ < nsteps_) {
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::set_porosity_task(
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::set_fabric_task(std::string fabric_type,
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::set_rotation_task(
...
template <unsigned Tdim>
bool mpm::MPMExplicit<Tdim>::update_state_task() {
  bool status = true;
  if (step_ < nsteps_) {
    // Phase
    const unsigned phase = 0;
