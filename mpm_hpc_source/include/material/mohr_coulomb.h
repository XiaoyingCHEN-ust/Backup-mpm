...
class MohrCoulomb : public Material<Tdim> {
 public:
  ...
  Eigen::Matrix<double, 6, 1> compute_stress(const Vector6d& stress, const Vector6d& dstrain,
  ...
  // Return plastic work
  double plastic_work() const override { return plastic_work_; }

  // Return deviatoric plastic strain increment
  double pdstrain() const override { return pdstrain_; }

  // Return yield type
  double yield_type() const override { return yield_type_; }  

  // Return cohesion
  double cohesion() const override { return cohesion_; }
