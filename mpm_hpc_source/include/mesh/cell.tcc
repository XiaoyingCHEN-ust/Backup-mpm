...
bool mpm::Cell<Tdim>::initialise() {
  ...
  try {
    ...
    if (this->nnodes_ == this->nodes_.size()) {
      ...
      // Shape function at the centroid
      shapefn_centroid_ =
          element_->shapefn(xi_centroid, zero, zero);
...
inline void mpm::Cell<2>::compute_normals() {
  ...
  for (unsigned face_id = 0; face_id < element_->nfaces(); ++face_id) {
    ...
    // the reverse does not work to produce normal that is positive pointing
    // out of the element
...
bool mpm::Cell<Tdim>::initialise_element_matrix() {
  ...
  if (this->status()) {
    try {
      ...
      L_element_.setZero();
      // Initialse pressure compressibility matrix (NxN)
      P_element_.resize(nnodes_, nnodes_);
      P_element_.setZero();   
      // Initialse stabilization matrix (NxN)
      S_element_.resize(nnodes_, nnodes_);
      S_element_.setZero();          
      ...
      // Initialse Ts matrix (NxN)
      T_element_.resize(nnodes_, nnodes_);
      T_element_.setZero();    
      // Initialse Pw matrix (NxN)
      P_w_element_.resize(nnodes_, nnodes_);
      P_w_element_.setZero();
      ...
      average_shapefn_.resize(nnodes_, 1);
      average_shapefn_.setZero();

      // Initialse Laplacian matrix (NxN)
      KTT_element_.resize(nnodes_, nnodes_);
      KTT_element_.setZero();
...
//! Compute average shape function
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_average_shapefn(const Eigen::VectorXd& shapefn,
                                              double pvolume) noexcept {
  // Lock the storage
  std::lock_guard<std::mutex> guard(cell_mutex_);
  average_shapefn_ += shapefn / particles_.size();     
}

//! Compute local matrix of laplacian
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_L_element(const Eigen::VectorXd& shapefn,
  ...
  std::lock_guard<std::mutex> guard(cell_mutex_);
// success 2  
  L_element_ += grad_shapefn * grad_shapefn.transpose() * multiplier_s * pvolume + 
                grad_shapefn * grad_shapefn.transpose() * multiplier_l * pvolume;
  P_element_ += alpha_liquid * pvolume * shapefn * (shapefn.transpose());  
}

//! Compute local matrix of laplacian
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_stab_element(const Eigen::VectorXd& shapefn,
                                        const Eigen::VectorXd& shapefn_centroid,
                                        double stab_para) noexcept {
  // Lock the storage
  std::lock_guard<std::mutex> guard(cell_mutex_); 
  S_element_ += (shapefn - shapefn_centroid) * (shapefn.transpose() - shapefn_centroid.transpose()) * stab_para;
}

//! Compute element F_s_element and F_m_element
//! Used in poisson equation RHS
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_F_element(const Eigen::VectorXd& shapefn,
                                        const Eigen::MatrixXd& grad_shapefn,
                                        double pvolume,
                                        double multiplier_s,
  ...
  for (unsigned i = 0; i < Tdim; i++) {
    // F_s_element
    F_s_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
        shapefn * grad_shapefn.col(i).transpose() * pvolume * multiplier_s;
    // F_m_element
    F_m_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
        grad_shapefn.col(i) * shapefn.transpose() * pvolume * multiplier_l;
// success 2    
    // F_s_element
    // F_s_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
    //     grad_shapefn.col(i) * shapefn.transpose() * pvolume * multiplier_s;
    // // F_m_element
    // F_m_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
    //     grad_shapefn.col(i) * shapefn.transpose() * pvolume * multiplier_l; 
    }
...
//! Used in poisson equation RHS
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_T_element(const Eigen::VectorXd& shapefn,
                                        double pvolume,
  ...
  // Compute components at nodes
    ...
    // T_element
    T_element_ += 3 * beta * pvolume * shapefn * (shapefn.transpose()); 
}

//! Compute element T_element
//! Used in poisson equation RHS
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_P_element(const Eigen::VectorXd& shapefn,
                                        double pvolume,
  ...
  // Compute components at nodes
    ...
    // T_element
    P_w_element_ += multiplier * pvolume * shapefn * (shapefn.transpose()); 
}

//! Compute element K_cor_w_element_
//! Used in compute correct nodal velocity
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_K_cor_element(const Eigen::VectorXd& shapefn,
                                            const Eigen::MatrixXd& grad_shapefn,
                                            double pvolume,
                                            double multiplier_s,
                                            double multiplier_l,
                                            double radius,
                                            bool is_axisymmetric) noexcept {

  std::lock_guard<std::mutex> guard(cell_mutex_);
  for (unsigned i = 0; i < Tdim; i++) {
    // Compute K_cor_s_element_ and K_cor_w_element_
    // K_cor_s_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
    //    grad_shapefn.col(i) * shapefn.transpose() * multiplier_s * pvolume;
    // K_cor_w_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
    //    grad_shapefn.col(i) * shapefn.transpose() * multiplier_l * pvolume;

    // Compute K_cor_s_element_ and K_cor_w_element_
    K_cor_s_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
       shapefn * grad_shapefn.col(i).transpose() * multiplier_s * pvolume;
    K_cor_w_element_.block(0, i * nnodes_, nnodes_, nnodes_) +=
       shapefn * grad_shapefn.col(i).transpose() * multiplier_l * pvolume;

  }

  if (is_axisymmetric) {
      K_cor_s_element_.block(0, 0, nnodes_, nnodes_) +=
          shapefn * shapefn.transpose() / radius * multiplier_s * pvolume;
      K_cor_w_element_.block(0, 0, nnodes_, nnodes_) +=
          shapefn * shapefn.transpose() / radius * multiplier_l * pvolume;
  }
}
...
bool mpm::Cell<Tdim>::map_cell_volume_to_nodes(unsigned phase) {
  ...
  try {
    ...
    for (unsigned i = 0; i < nodes_.size(); ++i) {
      nodes_[i]->update_volume(true, phase, volume_ / nnodes_, mean_length_ / nnodes_);
    }
...
//! Compute local matrix of laplacian
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_KTT_element(const Eigen::VectorXd& shapefn,
                                        const Eigen::MatrixXd& grad_shapefn,
                                        double pvolume,
  ...
  std::lock_guard<std::mutex> guard(cell_mutex_);
// success 2  
  KTT_element_ += grad_shapefn * grad_shapefn.transpose() * beta_m * pvolume;
}
